		...
		if ("stage-status".equals(request.requestName())) { // server is informing about a status change.
			...
			return new DefaultGoPluginApiResponse(200, new Gson().toJson(new MapBuilder<>(new HashMap<String,Object>())
				...
				.put("clientSecret", new MapBuilder<>(new HashMap<String,Object>())
					...
					.put("secure", true)
					.build())
				.put("clientSecretError", new MapBuilder<>(new HashMap<String,Object>())
					.put("display-name", "Client Secret Error")
					.put("display-order", "3")
					.put("required", false)
					.build())
				...
				.put("goPassword", new MapBuilder<>(new HashMap<String,Object>())
					...
					.put("secure", true)
					.build())
				.put("goPasswordError", new MapBuilder<>(new HashMap<String,Object>())
					.put("display-name", "Go API Password Error")
					.put("display-order", "6")
					.put("required", false)
					.build())
				.build()));
